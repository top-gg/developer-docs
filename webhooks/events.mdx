...
<Tabs>
  ...
  <Tab title="Server webhook">
    | Field | Type | Description |
    |---|---|---|
    | `guild` | string | Discord ID of the guild that was voted for |
    | `user` | string | Discord ID of the user who voted |
    | `type` | string | `"upvote"` for real votes, `"test"` when triggered from the dashboard |
    | `query` | string (optional) | Query string parameters appended to the `/servers/:id/vote` page URL |

    ```json
    {
      "guild": "803190510032756736",
      "user": "160105994217586689",
      "type": "upvote",
      "query": ""
    }
