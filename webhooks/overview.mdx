...
3. Reject the request if the timestamp is more than 5 minutes old, to guard against replay attacks.
4. Compute an HMAC SHA-256 digest of `{timestamp}.{rawBody}` using your webhook secret as the key.
5. Compare the computed digest to the `v1` value using a constant-time comparison.
