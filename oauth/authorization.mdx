...
<ParamField query="platform" type="string">
  Platform for `platform_ids`, for example `discord`. Use both together when you know the external ID but not the Top.gg project ID.
</ParamField>

<ParamField query="platform_ids" type="string">
  Comma-separated external IDs on `platform`, for example Discord bot IDs. Matching projects are preselected on the consent screen, same behavior as `project_ids`. Unknown IDs are ignored.
</ParamField>
