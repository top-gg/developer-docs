...
Scopes limit what your application can do with an access token. Request them space-separated in the `scope` parameter of the [authorization request](/oauth/authorization); the user sees each one on the consent screen. Every endpoint states its required scope; a request without it returns `403 Forbidden`.
