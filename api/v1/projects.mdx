...
The metrics payload is shaped per platform. For `POST /projects/@me/metrics`, send this payload as the entire request body. For the batch metrics endpoint, send the same payload under `data[].metrics`. Sending the wrong shape for your project returns `422`.
