...
The projects endpoints let you retrieve a project's details, update its headline and page content, create announcements, and push updated command definitions to Top.gg. `:project_id` is the Top.gg project ID of a project your [OAuth authorization](/oauth/overview) covers; each endpoint states its required [scope](/oauth/scopes).
...
**OAuth scope:** none, any valid access token

Lists the projects the user granted your application. The set only changes when the user goes through authorization again, so fetch it once after the token exchange and store it. Use the returned `id` on the project endpoints.
