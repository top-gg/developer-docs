...
```bash
curl -X POST https://top.gg/api/v1/projects/@me/metrics \
  -H "Authorization: Bearer $TOPGG_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "server_count": 2133,
...
Submits up to 100 metrics entries in a single request. Entries are applied in ascending timestamp order, so you can backfill gaps or push buffered samples after a network interruption.
    ...
    "data": [
      {
        ...
        "metrics": { "server_count": 2130 }
      },
      {
        "timestamp": "2026-04-17T10:05:00Z",
        "metrics": { "server_count": 2133, "shard_count": 8 }
      }
    ]
  }'
```
...
The `metrics` body on both metrics endpoints is shaped per platform. Sending the wrong shape for your project returns `422`.
