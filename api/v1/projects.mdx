...
description: "Use the /projects/:project_id endpoints to retrieve project details, update headline and page content, create announcements, and update Discord slash commands. Requires a valid Bearer token."
---

The projects endpoints let you retrieve a project's details, update its headline and page content, create announcements, and push updated command definitions to Top.gg. `:project_id` is the Top.gg project ID of a project your [OAuth authorization](/oauth/overview) covers, and each endpoint states its required [scope](/oauth/scopes).
...
| `PATCH /projects/:project_id` | `PATCH /projects/@me` |
| `POST /projects/:project_id/announcements` | `POST /projects/@me/announcements` |
| `PATCH /projects/:project_id/metrics` | `PATCH /projects/@me/metrics` |
| `POST /projects/:project_id/metrics/batch` | `POST /projects/@me/metrics/batch` |
| `PUT /projects/:project_id/commands` | `PUT /projects/@me/commands` |
...
**OAuth scope:** none, any valid access token or [application token](/oauth/application-token)

Lists the projects the current credential covers. With an access token that is the single project of the authorization. With an application token it is every project granted to your application, 100 per page.

Not available with project tokens.

```bash
curl https://top.gg/api/v1/projects \
...
<ResponseField name="cursor" type="string">
  Present while more pages exist. Pass it back as the `cursor` query parameter. Keep paging until the cursor is absent. A page can be empty while later pages still hold projects.
</ResponseField>
...
```bash
curl -X PATCH https://top.gg/api/v1/projects/218109768489992192 \
  -H "Authorization: Bearer $TOPGG_TOKEN" \
...
```bash
curl -X POST https://top.gg/api/v1/projects/218109768489992192/announcements \
  -H "Authorization: Bearer $TOPGG_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH https://top.gg/api/v1/projects/218109768489992192/metrics \
  -H "Authorization: Bearer $TOPGG_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "server_count": 420,
...
```bash
curl -X POST https://top.gg/api/v1/projects/218109768489992192/metrics/batch \
  -H "Authorization: Bearer $TOPGG_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "data": [
...
The metrics payload is shaped per platform. For `PATCH /projects/:project_id/metrics`, send this payload as the request body. For the batch metrics endpoint, send the same payload under `data[].metrics`. Sending the wrong shape for your project returns `422`.
...
```bash
curl -X PUT https://top.gg/api/v1/projects/218109768489992192/commands \
  -H "Authorization: Bearer $TOPGG_TOKEN" \
  -H "Content-Type: application/json" \
